...
## Claude Code notes
- **Model tiers: Lead uses balanced (Sonnet) for coordination and deviation analysis; builder subagents use fast (Haiku) for scoped implementation.** `/fast` mode is appropriate for the lead.
- Respect `builder_tier` from `plan.json` task groups — use Opus for `high_reasoning` groups, Sonnet for `balanced` groups, Haiku for `fast` groups.
